// RUN TO 12 TO GET CORRECT ORIENTATION
...
void setup() {
  ...
  //re-set the origin of the turtle 
  xPosition = width/2;
  yPosition = height;
...
void turtleDraw() {
  ...
  for (currentCharacter= 0; currentCharacter < newWord.length(); currentCharacter++) {
    ...
    if ( newWord.charAt(currentCharacter) == '+') {
      ...
      //we are pushing (saving) the origin location and rotation information onto the stack 
      pushMatrix();
    }
    else if ( newWord.charAt(currentCharacter) == ']') {
      // we are poping (removing) the most recent saved location/rotation information from the stack 
      popMatrix();
    }
    else if ( newWord.charAt(currentCharacter) == '6') {
      //make colour 
      stroke (53, 71, 100);
    }
    else if ( newWord.charAt(currentCharacter) == '5') {
      //make colour 
      stroke (360, 69, 100);
    }
    else if ( newWord.charAt(currentCharacter) == '4') {
      //make colour 
      stroke (26, 100, 100);
    }
